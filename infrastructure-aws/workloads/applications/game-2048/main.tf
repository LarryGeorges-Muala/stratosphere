...
data "aws_lb_target_group" "game_2048" {
  for_each = tomap(local.disaster_recovery)
  region   = each.key
...
resource "helm_release" "game_2048" {
  ...
  chart = "../../devops/charts/game-2048/charts/game-2048-0.1.0.tgz"
  ...
  set = [
    ...
    },
    {
      ...
      value = "${data.aws_lb_target_group.game_2048[local.main_region].arn}"
    },
    {
...
resource "helm_release" "game_2048_disaster_recovery" {
  ...
  set = [
    ...
    {
      name  = "targetGroupBinding.arn"
      value = "${data.aws_lb_target_group.game_2048[local.recovery_region].arn}"
    },
    {
      name  = "targetGroupBinding.securityGroup"
