...
resource "kubernetes_secret" "prometheus_config" {
  ...
  data = {
    "prometheus.yml" = "${file("${path.module}/monitoring/prometheus/config/prometheus.yml")}"
  }
  type = "generic"
}
...
resource "kubernetes_secret" "prometheus_rules" {
  ...
  data = {
    "prometheus.rules" = "${file("${path.module}/monitoring/prometheus/rules/prometheus.rules")}"
  }
  type = "generic"
}
