...
### kubectl describe daemonset/alertmanager-monitoring -n staging
######
### kubectl get events -n staging
### kubectl port-forward svc/alertmanager-monitoring-api -n staging 9090:9090
### kubectl exec alertmanager-monitoring-dwkdn -n staging -it -- /bin/sh
######
###############################################################################
...
resource "kubernetes_secret" "alertmanager_config" {
  ...
  data = {
    "alertmanager.yml" = "${file("${path.module}/monitoring/alertmanager/config/alertmanager.yml")}"
  }
  type = "generic"
}
...
resource "helm_release" "alertmanager" {
  ...
  lint              = false
  timeout           = 900
  values            = [file("${path.module}/helm-values/alertmanager.yaml")]
}
